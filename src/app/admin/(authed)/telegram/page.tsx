...
export default function TelegramPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {(error || info) && (
        <div className={`flex items-start gap-2 px-4 py-3 rounded-lg text-sm border ${error ? "bg-[var(--danger-bg)] border-[var(--danger-bg)] text-[var(--danger-text)]" : "bg-[var(--success-bg)] border-[var(--success-bg)] text-[var(--success-text)]"}`} role="status">
      ...
      <div className="bg-[var(--card)] border border-[var(--color-border,#E2E8F0)] rounded-lg p-4">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="text-sm">
            ...
            {data && !data.configured && (
              <p className="mt-1 text-[var(--warning-text)]">Faltan las variables <code>TELEGRAM_BOT_TOKEN</code> / <code>TELEGRAM_WEBHOOK_SECRET</code> en el servidor.</p>
            )}
            {data?.configured && (
              <p className="mt-1 text-[var(--color-text-body,#475569)]">
                ...
                {data.webhook?.last_error_message ? <span className="text-[var(--danger-text)]"> Último error: {data.webhook.last_error_message}</span> : null}
              </p>
            )}
          </div>
          <button type="button" onClick={connect} disabled={connecting || !data?.configured}
            className="inline-flex items-center gap-1.5 rounded-lg border border-[var(--color-border,#E2E8F0)] px-3 py-1.5 text-sm font-medium hover:bg-[var(--muted)] disabled:opacity-50">
                ...
                {data.recipients.map((r) => (
                  <tr key={r.id} className="hover:bg-[var(--bg-subtle,#F8FAFC)]">
                    <td className="px-4 py-3">
                      ...
                      {r.chatId
                        ? <div className="text-xs text-[var(--success-text)] inline-flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> Vinculado</div>
                        : <PairingHint code={r.pairingCode} expires={r.pairingExpiresAt} bot={data.botUsername} />}
                    </td>
                    <td className="px-4 py-3">
                      {r.active
                        ? <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-[var(--success-bg)] text-[var(--success-text)]">Activo</span>
                        : <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium bg-[var(--muted)] text-[var(--text-body)]">Inactivo</span>}
                    </td>
                    <td className="px-4 py-3">
                      <select value={r.canOperate ? "op" : "rx"} disabled={pendingId === r.id}
                        onChange={(e) => void patch(r, { canOperate: e.target.value === "op" })}
                        className="rounded-lg border border-[var(--color-border,#E2E8F0)] bg-[var(--card)] px-2 py-1 text-xs disabled:opacity-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <button type="button" disabled={pendingId === r.id} onClick={() => void patch(r, { active: !r.active })}
                          className="inline-flex items-center gap-1 text-xs text-[var(--text-body)] hover:text-[var(--text-dark)] disabled:opacity-50">
...
function PairingHint({ code, expires, bot }: { code: string | null; expires: string | null; bot: string | null }) {
  ...
  if (!code) return <div className="text-xs text-[var(--text-muted)]">Pendiente de vincular</div>;
  ...
  return (
    <div className="text-xs text-[var(--color-text-body,#475569)] mt-0.5">
      Código: <code className="rounded bg-[var(--muted)] px-1 font-mono">{code}</code>
      <button type="button" onClick={() => { navigator.clipboard?.writeText(code); setCopied(true); }} className="ml-1 text-[var(--text-muted)] hover:text-[var(--primary)]" title="Copiar">
        <Copy className="inline h-3 w-3" />
      </button>
      {copied && <span className="ml-1 text-[var(--success-text)]">copiado</span>}
      {caducado
        ? <span className="ml-1 text-[var(--warning-text)]">(caducado — elimina y vuelve a añadir)</span>
        : <span className="ml-1 text-[var(--text-muted)]">{bot ? <>· que lo envíe a <a className="underline" href={`https://t.me/${bot}`} target="_blank" rel="noreferrer">@{bot}</a></> : "· que lo envíe al bot"}</span>}
    </div>
  );
}
...
function AddModal({ onClose, onAdded }: { onClose: () => void; onAdded: (label: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4" role="dialog" aria-modal="true">
      <form onSubmit={submit} className="bg-[var(--card)] rounded-lg shadow-xl max-w-md w-full p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-[var(--text-dark)]">Añadir persona</h2>
            <p className="text-sm text-[var(--text-body)] mt-1">Se genera un código de un solo uso (15 min). La persona lo envía al bot para vincularse.</p>
          </div>
        </div>
        <div>
          <label htmlFor="tg-label" className="block text-sm font-medium text-[var(--text-dark)] mb-1.5">Nombre</label>
          <input id="tg-label" required autoFocus value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Dani, Borja…"
            className="flex h-10 w-full rounded-lg border border-[var(--color-border,#E2E8F0)] bg-[var(--card)] px-3.5 py-2 text-sm focus-visible:outline-none focus-visible:border-[var(--primary)] focus-visible:ring-2 focus-visible:ring-[var(--primary)]/20" />
        </div>
        <label className="flex items-center gap-2 text-sm text-[var(--text-body)]">
          <input type="checkbox" checked={canOperate} onChange={(e) => setCanOperate(e.target.checked)} className="h-4 w-4 rounded border-[var(--border-strong)] text-[var(--primary)]" />
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button type="button" onClick={onClose} disabled={submitting} className="px-4 py-2 rounded-lg text-sm font-medium text-[var(--text-body)] hover:bg-[var(--muted)] disabled:opacity-50">Cancelar</button>
