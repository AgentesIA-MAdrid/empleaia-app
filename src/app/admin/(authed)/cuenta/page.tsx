...
export default function CuentaPage() {
  ...
  return (
    <div className="mx-auto max-w-md">
      <h1 className="mb-1 text-2xl font-bold text-[var(--text-dark)]">Mi cuenta</h1>
      <p className="mb-6 text-sm text-[var(--text-muted)]">Cambia la contraseña de tu acceso al panel super-admin.</p>

      <form onSubmit={submit} className="space-y-3 rounded-xl border border-[var(--border)] bg-[var(--card)] p-5">
        <div>
          <label className="text-sm font-medium text-[var(--text-body)]">Nueva contraseña</label>
          <input
            type="password"
            value={pw}
        ...
        <div>
          <label className="text-sm font-medium text-[var(--text-body)]">Repite la contraseña</label>
          <input
            type="password"
            value={pw2}
        ...
        {msg && (
          <p className={`text-sm ${msg.ok ? "text-[var(--success-text)]" : "text-[var(--danger-text)]"}`}>{msg.text}</p>
        )}
        <button
          type="submit"
          disabled={busy}
