  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-dark)]">Artículos</h1>
          <p className="text-[var(--text-muted)] text-sm mt-1">Base de conocimiento y recursos</p>
        </div>
          ...
          <button
            ...
            className={cn(
              ...
              categoriaFiltro === cat
                ...
                : "bg-[var(--card)] text-[var(--text-body)] border-[var(--border)] hover:border-[var(--primary)]"
            )}
          >
            {cat}
          </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((i) => <div key={i} className="h-40 bg-[var(--muted)] rounded-xl animate-pulse" />)}
        </div>
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {a.publicado
                        ? <span className="text-xs bg-[var(--success-bg)] text-[var(--success-text)] px-2 py-0.5 rounded-full">Publicado</span>
                        : <span className="text-xs bg-[var(--muted)] text-[var(--text-muted)] px-2 py-0.5 rounded-full">Borrador</span>}
                    </div>
                    <h3 className="font-semibold text-[var(--text-dark)] truncate">{a.titulo}</h3>
                    <p className="text-sm text-[var(--text-muted)] mt-1 line-clamp-2">{a.contenido}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button className="p-1 text-[var(--text-muted)] hover:text-[var(--primary)] transition-colors" onClick={() => togglePublicado(a)}>
                      {a.publicado ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                    </button>
                    <button className="p-1 text-[var(--text-muted)] hover:text-[var(--danger)] transition-colors" onClick={() => handleDelete(a.id)}>
        ...
        <DialogContent className="max-w-[95vw] sm:max-w-lg">
          ...
          <div className="space-y-4 py-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-[var(--text-body)]">Publicar inmediatamente</span>
            </label>
          </div>
          <DialogFooter>
