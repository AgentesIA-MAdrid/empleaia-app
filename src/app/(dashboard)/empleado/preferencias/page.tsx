...
export default function PreferenciasPage() {
  ...
  const Toggle = ({ value, onChange }: { value: boolean; onChange: () => void }) => (
    ...
    >
      <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-[var(--card)] transition-transform ${value ? "translate-x-5" : "translate-x-0.5"}`} />
    </button>
  );

  if (!prefs) return <div className="p-6 animate-pulse"><div className="h-40 bg-[var(--muted)] rounded-xl" /></div>;

  return (
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-dark)]">Preferencias de notificaciones</h1>
        <p className="text-[var(--text-muted)] text-sm mt-1">Elige cómo y cuándo quieres recibir notificaciones</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {/* Header */}
          <div className="grid grid-cols-[1fr_auto_auto_auto] gap-2 sm:gap-4 px-4 sm:px-6 py-2 border-b bg-[var(--muted)] text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide">
            <span>Evento</span>
          ...
          <div className="divide-y">
            {EVENTOS.map(({ key, label, desc }) => {
              ...
              return (
                <div key={key} className="grid grid-cols-[1fr_auto_auto_auto] gap-2 sm:gap-4 px-4 sm:px-6 py-3 items-center hover:bg-[var(--muted)]">
                  <div>
