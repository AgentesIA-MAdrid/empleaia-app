...
import { FileText, Upload, Download, Trash2, PenLine, Loader2, CheckCircle2, Clock, FileCheck } from "lucide-react";
...
export function DocumentosEmpleadoTab({
  ...
  // probatoria de la firma (fecha, hash, IP, navegador). Los datos vienen de
  // /api/firmas; la fusión se hace en cliente (jsPDF + pdf-lib).
  const descargarFirmadoConActa = async (doc: DocRow) => {
  ...
  const estadoFirma = (doc: DocRow) => {
    if (doc.firmas && doc.firmas.length > 0) return { txt: "Firmado", cls: "text-[var(--success-text)]", icon: CheckCircle2 };
    const sol = doc.solicitudesFirma?.find((s) => s.destinatarioId === empleadoId);
    if (sol?.estado === "pendiente") return { txt: "Firma pendiente", cls: "text-[var(--warning-text)]", icon: Clock };
    return null;
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm text-[var(--text-muted)]">Documentos de {empleadoNombre} (nóminas, contratos, adjuntos…)</p>
        {puedeEnviar && (
      ...
      {loading ? (
        <div className="flex justify-center py-8"><Loader2 className="h-5 w-5 animate-spin text-[var(--text-muted)]" /></div>
      ) : docs.length === 0 ? (
        <p className="py-8 text-center text-sm text-[var(--text-muted)]">Sin documentos todavía.</p>
      ) : (
            ...
            return (
              <div key={d.id} className="flex items-center gap-3 px-3 py-2.5 text-sm">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium text-[var(--text-dark)]">{d.nombre}</div>
                ...
                {isSafeDocUrl(d.url) && (
                  <button type="button" onClick={() => downloadDoc(d.url, d.nombre)} className="text-[var(--text-muted)] hover:text-[var(--primary)]" title="Descargar documento">
                    <Download className="h-4 w-4" />
                  </button>
                )}
                {isSafeDocUrl(d.documentoRellenoUrl) && (
                  <button type="button" onClick={() => downloadDoc(d.documentoRellenoUrl, `${d.nombre} (con datos).pdf`)} className="text-[var(--text-muted)] hover:text-[var(--success-text)]" title="Descargar con los datos rellenados">
                    <FileText className="h-4 w-4" />
                  </button>
                )}
                {ef?.txt === "Firmado" && (
                  <button type="button" onClick={() => void descargarFirmadoConActa(d)} disabled={descargandoId === d.id} className="text-[var(--text-muted)] hover:text-[var(--success-text)] disabled:opacity-50" title="Descargar documento firmado con certificado">
                    {descargandoId === d.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileCheck className="h-4 w-4" />}
                  </button>
                )}
                {puedeFirmarYBorrar && isSafeDocUrl(d.url) && !ef && (
                  <button type="button" onClick={() => void solicitarFirma(d)} disabled={firmandoId === d.id} className="text-[var(--text-muted)] hover:text-[var(--primary)] disabled:opacity-50" title="Solicitar firma">
                    {firmandoId === d.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <PenLine className="h-4 w-4" />}
                  </button>
                )}
                {puedeFirmarYBorrar && (
                  <button type="button" onClick={() => void borrar(d)} className="text-[var(--text-muted)] hover:text-[var(--danger)]" title="Eliminar">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input ref={fileRef} type="file" accept=".pdf,image/*"
                ...
                className="mt-1 block w-full text-sm file:mr-3 file:rounded-md file:border-0 file:bg-[var(--muted)] file:px-3 file:py-1.5 file:text-sm" />
            </div>
            <div>
