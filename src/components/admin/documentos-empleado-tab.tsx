...
export function DocumentosEmpleadoTab({
  ...
  // certificado/acta probatorio de la firma. Los datos (documento sellado,
  // fecha, hash, IP, navegador) vienen de /api/firmas; la fusión se hace en
  // cliente.
          ...
          {docs.map((d) => {
            ...
            return (
              <div key={d.id} className="flex items-center gap-3 px-3 py-2.5 text-sm">
                ...
                {ef?.txt === "Firmado" && (
                  <button type="button" onClick={() => void descargarFirmadoConActa(d)} disabled={descargandoId === d.id} className="text-slate-400 hover:text-emerald-600 disabled:opacity-50" title="Descargar documento firmado + certificado (un solo PDF)">
